...
getOrgChart.prototype._az = function () {
    this._zA = get._r()
        .msie ? this.element.clientWidth : window.getComputedStyle(this.element,
            null)
        .width;
    ...
    this._zQ = get._r()
        .msie ? this.element.clientHeight : window.getComputedStyle(this.element,
            null)
        .height;
...
getOrgChart.prototype.changeColorScheme = function (a) {
    ...
    this._Q._zp.className = this._Q._zp.className.replace(this.config.color,
        a);
    this.config.color = a
};
    ...
    for (var b = 0; b < this._aB.length; b++) {
        ...
        switch (this.render) {
            case "SVG":
                ...
                f.push(getOrgChart.OPEN_GROUP.replace("[x]", j)
                    .replace("[y]", l)
                        ...
                        case "text":
                            ...
                            for (k = 0; k < this.config.primaryColumns.length; k++) {
                                ...
                                f.push(this.theme.text.replace("[index]", g)
                                    .replace("[text]", d.data[c])
                                    .replace("[y]", e.y)
                                    .replace("[x]", e.x)
                                    .replace("[rotate]", e.rotate)
                                    .replace("[width]", e.width));
                                g++
                            }
                            break
                    }
...
getOrgChart.DETAILS_VIEW_INPUT_HTML =
    '<div data-field-name="[label]"><div class="get-label">[label]</div><div class="get-data"><input value="[value]"/></div></div>';
...
getOrgChart.person.prototype.compareTo = function (b) {
    var a = this;
    if (a === undefined || b === undefined || a._zW === undefined || a._zS ===
        undefined || b._zW === undefined || b._zS === undefined) {
        return false
    } else {
...
get._w = function (a, c, b, h, j, d) {
    ...
    function f() {
        for (var s in b) {
            var t = getOrgChart.util._zZ(["top", "left", "right",
                "bottom"
            ], s.toLowerCase()) ? "px" : "";
    ...
    for (n = 0; n < this._aB.length; n++) {
        for (m in this._aB[n].data) {
            ...
            if (getOrgChart.util._af(this._aB[n]) && getOrgChart.util._af(
                this._aB[n].data[m])) {
                var c = this._aB[n].data[m].toString()
                    .toLowerCase();
                b = c.indexOf(e)
            }
            if (b > -1) {
...
getOrgChart.prototype.load = function () {
    ...
    if (a.constructor && (a.constructor.toString()
        .indexOf("HTML") > -1)) {
