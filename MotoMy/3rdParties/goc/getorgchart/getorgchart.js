getOrgChart = function (b, a) {
    ...
    this.element = b;
    this.render = (this.config.render == "AUTO") ? getOrgChart._C() : this.config
        .render;
    this._au = [];
    this._aj = [];
    this._aO = [];
...
getOrgChart.prototype._az = function () {
    ...
    this._zQ = get._r().msie ? this.element.clientWidth : window.getComputedStyle(
    ...
    this._zp = get._r().msie ? this.element.clientHeight : window.getComputedStyle(
    ...
    this.element.innerHTML = getOrgChart.INNER_HTML.replace("[theme]", this
        .config.theme).replace("[color]", this.config.color).replace(
        /\[height]/g, this._aF).replace(/\[toolbar-height]/g, this.theme
        .toolbarHeight)
...
getOrgChart.prototype.changeColorScheme = function (a) {
    ...
    this._Q._zl.className = this._Q._zl.className.replace(this.config.color,
        a);
    this.config.color = a
    ...
    for (var b = 0; b < this._aB.length; b++) {
        ...
        switch (this.render) {
            ...
            case "SVG":
                ...
                f.push(getOrgChart.OPEN_GROUP.replace("[x]", j).replace(
                    "[y]", l).replace("[level]", d.level));
                    ...
                    switch (themeProperty) {
                        ...
                        case "text":
                            ...
                            for (k = 0; k < this.config.primaryColumns.length; k++) {
                                ...
                                f.push(this.theme.text.replace("[index]", g)
                                    .replace("[text]", d.data[c]).replace(
                                        "[y]", e.y).replace("[x]", e.x)
                                    .replace("[rotate]", e.rotate).replace(
                                        "[width]", e.width));
                                g++
...
getOrgChart.prototype._a5 = function () {
    ...
    switch (this.render) {
        ...
        case "SVG":
            a.push(getOrgChart.OPEN_SVG.replace("[defs]", this.config.embededDefinitions)
                .replace("[viewBox]", b.toString()));
            a.push(this._p());
...
getOrgChart.prototype._4 = function () {
    ...
    if (this.config.scale === "auto") {
        ...
        var f = Math.abs(d) + Math.abs(b) + this.theme.size[0] + this.config
            .siblingSeparation;
        var a = Math.abs(e) + Math.abs(c) + this.theme.size[1] + this.config
            .levelSeparation;
...
getOrgChart.OPEN_SVG =
    '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" version="1.1" viewBox="[viewBox]"><defs>[defs]</defs><g>';
...
getOrgChart._s = function (u, n, g) {
    ...
    for (var d = u.config.maxDepth - g; a != null && b != null && c <= d;) {
        ...
        var t = (b._aM + h + u._L(b) + u.config.subtreeSeparation) - (a._aM +
            m);
        if (t > 0) {
...
getOrgChart._zk = function (a) {
    ...
    for (i = 0; i < a._zu.length; i++) {
        ...
        while (b > c) {
            ...
            a._zu[i].textContent = a._zu[i].textContent.substring(0, a._zu[
                i].textContent.length - 4);
            a._zu[i].textContent += "...";
...
getOrgChart.person.prototype.compareTo = function (b) {
    ...
    var a = this;
    if (a === undefined || b === undefined || a._zZ === undefined || a._zW ===
        undefined || b._zZ === undefined || b._zW === undefined) {
        ...
        switch (d.render) {
            ...
            case "SVG":
                switch (d.config.linkType) {
                    ...
                    case "M":
                        c.push('<path class="link"   d="M' + e + "," + i +
                            " " + f + "," + j + " " + g + "," + l +
                            " L" + h + "," + m + '"/>');
                        break;
                    case "B":
...
get._w = function (a, c, b, h, j, d) {
    ...
    function f() {
        ...
        for (var s in b) {
            ...
            var t = getOrgChart.util._zA(["top", "left", "right",
                "bottom"
            ], s.toLowerCase()) ? "px" : "";
            switch (s.toLowerCase()) {

                case "d":
                    var v = j(((n * e) - e) / h) * (b[s][0] - c[s][0]) +
                        c[s][0];
                    var w = j(((n * e) - e) / h) * (b[s][1] - c[s][1]) +
                        c[s][1];
                    for (z = 0; z < a.length; z++) {

                        a[z].setAttribute("d", a[z].getAttribute("d") +
                            " L" + v + " " + w)
                ...
                case "transform":
                    if (b[s]) {
                        ...
                        for (i in q) {
                            ...
                            r[i] = j(((n * e) - e) / h) * (p[i] - q[i]) +
                                q[i]

                        }
                        for (z = 0; z < a.length; z++) {

                            a[z].setAttribute("transform", "matrix(" +
                                r.toString() + ")")
...
get._w._ax = function (a) {
    ...
    return a < 0.5 ? 0.5 * Math.pow(2, 10 * (2 * a - 1)) : 0.5 * (-Math.pow(
        2, 10 * (-2 * a + 1)) + 2)
    ...
    if (a < (1 / 2.75)) {
        ...
        if (a < (2 / 2.75)) {
            ...
            if (a < (2.5 / 2.75)) {
                ...
                return 1 - (7.5625 * (a - 2.25 / 2.75) * (a - 2.25 / 2.75) +
                    0.9375)

            } else {

                return 1 - (7.5625 * (a - 2.625 / 2.75) * (a - 2.625 / 2.75) +
                    0.984375)
...
get._w._aw = function (a) {
    ...
    return a < 0.5 ? 0.5 * (4 * a * a * ((2.5949 + 1) * 2 * a - 2.5949)) :
        0.5 * ((2 * a - 2) * (2 * a - 2) * ((2.5949 + 1) * (2 * a - 2) + 2.5949) +
            2)
...
getOrgChart.util._E = function () {
    ...
    var a = function () {
        ...
        return Math.floor((1 + Math.random()) * 65536).toString(16).substring(
            1)
...
getOrgChart.prototype._zg = function (c) {
    ...
    var b = function (p, j, q) {
        ...
        var r = "<select " + k +
            'class="get-oc-select-parent"><option value="' + p +
        ...
        for (var l = 0; l < j.length; l++) {
            ...
            if (o.id == p) {
                ...
                r += '<option selected="selected" value="' + o.id +
                    '">' + s + "</option>"
    ...
    var a = function (k, j) {
        ...
        var m =
            '<select class="get-oc-labels"><option value="">--other--</option>';
        var l;
        for (i = 0; i < j.length; i++) {

            if (!getOrgChart.util._zA(k, j[i])) {

                l += '<option value="' + j[i] + '">' + j[i] +
                    "</option>"
    ...
    for (label in c.data) {
        ...
        d += getOrgChart.DETAILS_VIEW_INPUT_HTML.replace("[label]", label).replace(
            "[value]", c.data[label]);
        h.push(label)
...
getOrgChart.prototype._h = function (l, a) {
    ...
    var e = getOrgChart.DETAILS_VIEW_INPUT_HTML.replace("[label]", h).replace(
        "[value]", "");
...
getOrgChart.prototype._ak = function (f, d) {
    ...
    if (this._aR.dragStart) {
        ...
        var e = g[3] / this._aF;
        var c = h > e ? h : e;
        g[0] = -((this._aR.mouseLastX - this._aR.dragStart.x) * c) + this._aR
            .dragStart.viewBoxLeft;
        g[1] = -((this._aR.mouseLastY - this._aR.dragStart.y) * c) + this._aR
            .dragStart.viewBoxTop;
        g[0] = parseInt(g[0]);
        g[1] = parseInt(g[1]);
...
getOrgChart.prototype._ab = function (i) {
    ...
    if (this._zw.found.length) {
        ...
        k[4] = k[4] - ((this.theme.size[0] / 2) * (getOrgChart.HIGHLIGHT_SCALE_FACTOR -
            1));
        k[5] = k[5] - ((this.theme.size[1] / 2) * (getOrgChart.HIGHLIGHT_SCALE_FACTOR -
            1));
        get._w(this._Q._f, {
...
getOrgChart.prototype._k = function () {
    ...
    if ((this._zw.showIndex < this._zw.found.length - 1) && (this._zw.found
        .length != 0)) {

        this._Q._aQ.className = this._Q._aQ.className.replace(
            " get-disabled", "")
        ...
        this._Q._aI.className = this._Q._aI.className.replace(
            " get-disabled", "")
...
getOrgChart.prototype._W = function (e) {
    ...
    for (n = 0; n < this._aB.length; n++) {
        ...
        for (m in this._aB[n].data) {
            ...
            var b = -1;
            if (getOrgChart.util._af(this._aB[n]) && getOrgChart.util._af(
                this._aB[n].data[m])) {
    ...
    if (b == "") {
        ...
        for (t = this._aB.length - 1; t >= 0; t--) {
            ...
            if (this._aB[t].id == b) {
                ...
                if (this._aB[t].pid == null || typeof (this._aB[t].pid) ==
                    "undefined" || this._aB[t].pid == "") {
        ...
        if (!get._r().msie && c == "xml" && g.readyState == 4) {
            ...
            if (get._r().msie && c == "xml" && g.readyState == 4) {
                ...
                try {
                    ...
                    var i = new DOMParser();
                    var j = i.parseFromString(g.responseText,
                        "text/xml");
                    a(j)
...
(function (a) {
    ...
    a.fn.getOrgChart = function (e) {
        ...
        var b = ((arguments.length > 1) || ((arguments.length == 1) &&
